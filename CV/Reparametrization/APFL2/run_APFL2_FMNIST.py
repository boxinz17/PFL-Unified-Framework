...
import data_load
from APFL2_loss import loss_fn, test_accu_fn, fmj_grads
from APFL2_loss import loss_fn_norescale, test_accu_fn_norescale, fmj_grads_norescale
from APFL2_algorithms import train_ASCD
...
mathcal_L_w = (lambda_global + max(alpha_list)**2) * mathcal_L_prime
mathcal_L_beta = ((1 - min(alpha_list))**2) * mathcal_L_prime / M
...
print("Training by ASCD-PFL")
print("theta: {}".format(theta))
loss_ASCD_norescale, test_ASCD_norescale, _, _ = train_ASCD(loss_fn_norescale, test_accu_fn_norescale, fmj_grads_norescale, w0, beta0, n_commun, devices_train_list, 
train_loader_list, val_loader_list, lambda_global, alpha_list, eta, p_w, theta, nu, gamma, repo_step, device, rd_seed=rd_seed)
...
with open("./result/Noreparm_test_" + obj + "_" + data_name + "_ASCD.txt", "wb") as f:   #Pickling
    pickle.dump(test_ASCD_norescale, f)
