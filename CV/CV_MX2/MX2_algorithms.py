...
def train_LSGD(loss_fn, test_accu_fn, fmj_grads_LSGD, w0, beta0, sync_step, n_commun, devices_train_list, train_loader_list, val_loader_list,
               lambda_penal, repo_step, eta, device, rd_seed=111):
    ...
    train_loss = []
    test_accu = []
    train_loss.append(loss_fn(w0, beta0, train_loader_list, lambda_penal, device))
    ...
    while True:
        ...
        if k_iter % sync_step == 0:
            ...
            # Compute the loss
            train_loss.append(loss_fn(w_avg, beta, train_loader_list, lambda_penal, device))
        ...
        # Compute gradients
        w_grads, beta_grads = fmj_grads_LSGD(w_list, beta, index_choice, devices_train_list, lambda_penal, device)
...
def train_SCD(loss_fn, test_accu_fn, fmj_grads, w0, beta0, n_commun, devices_train_list, train_loader_list, val_loader_list, \
             lambda_penal, repo_step, eta, p_w, device, rd_seed=111):
    np.random.seed(rd_seed)
    ...
    while True:
        ...
        # Compute gradients
        fmj_grads_result = fmj_grads(w, beta, index_choice, devices_train_list, lambda_penal, device)
        ...
        if xi == 1:
            ...
            # Compute the loss
            train_loss.append(loss_fn(w, beta, train_loader_list, lambda_penal, device))
...
def train_SVRCD(loss_fn, test_accu_fn, F_grads, fmj_grads, w0, beta0, n_commun, devices_train_list, train_loader_list, val_loader_list, lambda_penal, \
    ...
    fgrv = F_grads(w_v, beta_v, train_loader_list, lambda_penal, device)
    ...
    while True:
        ...
        # Compute gradients
        fmj_grads_result_v = fmj_grads(w_v, beta_v, index_choice, devices_train_list, lambda_penal, device)
        fmj_grads_result_y = fmj_grads(w_y, beta_y, index_choice, devices_train_list, lambda_penal, device)
        ...
        if zeta == 1:
            ...
            train_loss.append(loss_fn(w_y, beta_y, train_loader_list, lambda_penal, device))
...
def train_ASVRCD(loss_fn, test_accu_fn, F_grads, fmj_grads, w0, beta0, n_commun, devices_train_list, train_loader_list, val_loader_list, lambda_penal, \
    ...
    while True:
        ...
        # Compute gradients
        fmj_grads_result_v = fmj_grads(w_v, beta_v, index_choice, devices_train_list, lambda_penal, device)
        fmj_grads_result_x = fmj_grads(w_x, beta_x, index_choice, devices_train_list, lambda_penal, device)
...
def train_ASCD(loss_fn, test_accu_fn, fmj_grads, w0, beta0, n_commun, devices_train_list, train_loader_list, val_loader_list, lambda_penal, \
