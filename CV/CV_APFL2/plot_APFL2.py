...
for i, obj in enumerate(obj_list):
    for j, dataset in enumerate(dataset_list):
        ...
        axes[i, j].plot(loss_results[i][j][1], color='r', linestyle='-', label='ASCD-PFL')
        axes[i, j].plot(loss_results[i][j][2], color='g', linestyle='-', label='ASVRCD-PFL')
...
plt.savefig('./result/APFL2_train.png', dpi=400, bbox_inches='tight')
...
for i, obj in enumerate(obj_list):
    for j, dataset in enumerate(dataset_list):
        axes[i, j].plot(test_results[i][j][0], color='b', linestyle='-', label='LSGD-PFL')
        axes[i, j].plot(test_results[i][j][1], color='r', linestyle='-', label='ASCD-PFL')
        axes[i, j].plot(test_results[i][j][2], color='g', linestyle='-', label='ASVRCD-PFL')
        axes[i, j].tick_params(axis='both', which='major', labelsize=19)
        if j == 0:
...
plt.savefig('./result/APFL2_test.png', dpi=400, bbox_inches='tight')