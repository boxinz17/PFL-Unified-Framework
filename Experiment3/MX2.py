...
def loss_fn(w, beta_list, train_loader_list, lambda_penal):
    ...
    for m in range(M):
        ...
        for imgs, labels in train_loader:
            ...
            # Add all three terms
            with torch.no_grad():
                loss = loss_local + (lambda_penal/2) * loss_penalty
...
def F_grads(w, beta_list, train_loader_list, lambda_penal):
    # Define loss functions
    loss_CE_fn = nn.CrossEntropyLoss()
    penalty_fn = nn.MSELoss(reduction='sum')
    ...
    for m in range(M):
        ...
        for imgs, labels in train_loader:
            ...
            # Add all three terms
            loss_device += loss_local + (lambda_penal/2) * loss_penalty
...
def fmj_grads(w, beta_list, index_choice, devices_train_list, lambda_penal):
...
def train_GD(w0, beta0, n_epochs, train_loader_list, \
    ...
    for epoch in range(1, n_epochs + 1):
        # Compute gradients
        F_grads_result = F_grads(w, beta_list, train_loader_list, lambda_penal)
        ...
        # Compute the loss
        loss = loss_fn(w, beta_list, train_loader_list, lambda_penal)
        loss_GD.append(loss)
...
def train_SGD(w0, beta0, n_epochs, devices_train_list, train_loader_list, \
              lambda_penal, repo_step, eta, obj, data_name, rd_seed=111):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        avg_fmj_grads_result = fmj_grads(w_curr, beta_curr, 
                                         index_choice, devices_train_list, lambda_penal)
        ...
        # Compute the loss
        loss = loss_fn(w_curr, beta_curr, train_loader_list, lambda_penal)
...
def train_CD(w0, beta0, n_communs, devices_train_list, train_loader_list, \
             lambda_penal, repo_step, eta, prob1, obj, data_name, rd_seed=111):
    np.random.seed(rd_seed)
    ...
    while num_commun < n_communs:
        ...
        avg_fmj_grads_result = fmj_grads(w_curr, beta_curr, index_choice, 
                                         devices_train_list, lambda_penal)
...
def train_CDVR_commun(w0, beta0, n_communs, devices_train_list, train_loader_list, \
                      lambda_penal, repo_step, eta, prob1, obj, data_name, rho, rd_seed=111):
    np.random.seed(rd_seed)
    ...
    while num_commun < n_communs:
        ...
        if iter_num == 1 or xii == 1:
            F_grads_result_v = F_grads(w_v_curr, beta_v_curr, 
            train_loader_list, lambda_penal)
        ...
        if xi == 1:
            ...
            # Compute the loss
            loss = loss_fn(w_y_curr, beta_y_curr, train_loader_list, lambda_penal)
...
def train_CDVR_iter(w0, beta0, n_epochs, devices_train_list, train_loader_list, \
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        # Compute the loss
        loss = loss_fn(w_y_curr, beta_y_curr, 
        train_loader_list, lambda_penal)
        loss_CDVR.append(loss)
...
def loss_fn_local_sgd(w_list, beta_list, train_loader_list, lambda_penal):
    # Define loss functions
    loss_CE_fn = nn.CrossEntropyLoss()
    penalty_fn = nn.MSELoss(reduction='sum')
...
def fmj_grads_local_sgd(w_list, beta_list, index_choice, devices_train_list, lambda_penal):
    loss_CE_fn = nn.CrossEntropyLoss()
    penalty_fn = nn.MSELoss(reduction='sum')
...
def train_local_sgd(w_list0, beta_list0, sync_step, n_communs, devices_train_list, train_loader_list, 
    ...
    while num_commun < n_communs:
        ...
        if iter_num == 1 or iter_num % sync_step == 0:
            ...
            # Compute the loss
            loss = loss_fn_local_sgd(w_list_curr, beta_list_curr, train_loader_list, lambda_penal)
