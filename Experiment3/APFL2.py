...
def loss_fn(w, beta_list, train_loader_list, lambda_global, alpha_list):
    ...
    for m in range(M):
        ...
        for imgs, labels in train_loader:
            ...
            with torch.no_grad():
                outs_local = (1-alpha_m) * local_model(imgs.view(imgs.shape[0], -1)) + \
                alpha_m * global_model(imgs.view(imgs.shape[0], -1))
                loss_local = loss_CE_fn(outs_local, labels)
                
            # Add all terms
            with torch.no_grad():
                loss = lambda_global * loss_global + loss_local
...
def F_grads(w, beta_list, train_loader_list, lambda_global, alpha_list):
    # Define loss functions
    loss_CE_fn = nn.CrossEntropyLoss()
    penalty_fn = nn.MSELoss(reduction='sum')
    ...
    for m in range(M):
        ...
        for imgs, labels in train_loader:
            ...
            # Add all terms
            loss_device += lambda_global * loss_global + loss_local
...
def fmj_grads(w, beta_list, index_choice, devices_train_list, lambda_global, alpha_list):
    ...
    for m in range(M):
        ...
        # Compute loss of local model
        out_local = (1-alpha_m) * local_model(img.view(img.shape[0], -1)) + \
        alpha_m * global_model(img.view(img.shape[0], -1))
...
def train_GD(w0, beta0, n_epochs, train_loader_list, lambda_global, \
             repo_step, eta, alpha_list, obj, data_name):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if epoch == 1 or epoch % repo_step == 0:
            ...
            print("epoch: {}, loss: {:.10f}, time pass: {}s | GD {} {}".format(epoch, loss, pass_time, obj, data_name))
            
    return loss_GD, w_curr, beta_curr

def train_SGD(w0, beta0, n_epochs, devices_train_list, train_loader_list, \
              lambda_global, repo_step, eta, alpha_list, obj, data_name, rd_seed=111):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if epoch == 1 or epoch % repo_step == 0:
            now_time = datetime.datetime.now()
            pass_time = (now_time - start_time).seconds
            print("epoch: {}, loss: {:.10f}, time pass: {}s | SGD {} {}".format(epoch, loss, pass_time, obj, data_name))
            
    return loss_SGD, w_curr, beta_curr

def train_CD(w0, beta0, n_communs, devices_train_list, train_loader_list, \
             lambda_global, repo_step, eta, prob1, alpha_list, obj, data_name, rd_seed=111):
    np.random.seed(rd_seed)
    ...
    while num_commun < n_communs:
        ...
        avg_fmj_grads_result = fmj_grads(w_curr, beta_curr, index_choice, 
                                         devices_train_list, lambda_global, alpha_list)
        ...
        if xi == 1:
            ...
            # Compute the loss
            loss = loss_fn(w_curr, beta_curr, train_loader_list, lambda_global, alpha_list)
            loss_CD.append(loss)
        else:
            # Update beta
            beta_new = []
        ...
        if num_commun == 1 or num_commun % repo_step == 0:
            if num_commun not in reported_commun:
                ...
                now_time = datetime.datetime.now()
                pass_time = (now_time - start_time).seconds
                print("num_commun: {}, loss: {:.10f}, time pass: {}s | CD {} {}".format(num_commun, loss, pass_time, obj, data_name))
            
    return loss_CD, w_curr, beta_curr

def train_CDVR_commun(w0, beta0, n_communs, devices_train_list, train_loader_list, \
                      lambda_global, repo_step, eta, prob1, rho, alpha_list, obj, data_name, rd_seed=111):
    np.random.seed(rd_seed)
    ...
    while num_commun < n_communs:
        ...
        if xi == 1:
            ...
            # Compute the loss
            loss = loss_fn(w_y_curr, beta_y_curr, train_loader_list, lambda_global, alpha_list)
        ...
        if num_commun == 1 or num_commun % repo_step == 0:
            if num_commun not in reported_commun:
                ...
                now_time = datetime.datetime.now()
                pass_time = (now_time - start_time).seconds
                print("num_commun: {}, loss: {:.10f}, time pass: {}s | CDVR {} {}".format(num_commun, loss, pass_time, obj, data_name))
            
    return loss_CDVR, w_y_curr, beta_y_curr

def train_CDVR_iter(w0, beta0, n_epochs, devices_train_list, train_loader_list, \
                    lambda_global, repo_step, eta, prob1, rho, alpha_list, obj, data_name, rd_seed=111):
    np.random.seed(rd_seed)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        # Compute the loss
        loss = loss_fn(w_y_curr, beta_y_curr, train_loader_list, 
                       lambda_global, alpha_list)
        loss_CDVR.append(loss)
        ...
        if epoch == 1 or epoch % repo_step == 0:
            now_time = datetime.datetime.now()
            pass_time = (now_time - start_time).seconds
            print("epoch: {}, loss: {:.10f}, time pass: {}s | CDVR {} {}".format(epoch, loss, pass_time, obj, data_name))
            
    return loss_CDVR, w_y_curr, beta_y_curr

def loss_fn_local_sgd(w_list, beta_list, train_loader_list, lambda_global, alpha_list):
    # Define loss functions
    loss_CE_fn = nn.CrossEntropyLoss()
    penalty_fn = nn.MSELoss(reduction='sum')
...
def fmj_grads_local_sgd(w_list, beta_list, index_choice, devices_train_list, lambda_global, alpha_list):
    loss_CE_fn = nn.CrossEntropyLoss()
    penalty_fn = nn.MSELoss(reduction='sum')
...
def train_local_sgd(w_list0, beta_list0, alpha_list, sync_step, n_communs, devices_train_list, train_loader_list, 
                    lambda_global, repo_step, eta, obj, data_name, rd_seed=111):
    np.random.seed(rd_seed)

    eta /= sync_step  # Division based on theory
    ...
    while num_commun < n_communs:
        ...
        if iter_num == 1 or iter_num % sync_step == 0:
            ...
            # Compute the loss
            loss = loss_fn_local_sgd(w_list_curr, beta_list_curr, train_loader_list, lambda_global, alpha_list)
        ...
        if num_commun == 1 or num_commun % repo_step == 0:
            if num_commun not in reported_commun:
                ...
                now_time = datetime.datetime.now()
                pass_time = (now_time - start_time).seconds
                print("num_commun: {}, loss: {:.15f}, time pass: {}s | Local_SGD {} {}".format(num_commun, loss, pass_time, obj, data_name))
